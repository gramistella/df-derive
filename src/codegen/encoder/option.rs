...
//!
//! The `option_for_*` helpers turn a [`LeafBuilder`] into an `[Option]`
//! shape `Encoder::Leaf` by switching the storage layout (e.g. `Vec<T>` →
//! `Vec<Option<T>>`, MBVA → MBVA + validity bitmap, `Vec<bool>` →
//! bit-packed values bitmap + validity bitmap), reusing the builder's
...
use super::idents;
use super::leaf::{
    LeafBuilder, mb_decl, mb_decl_filled, mbva_decl, row_idx_decl, validity_into_option, vec_decl,
};
...
/// `<Chunked>::with_chunk`.
fn option_for_numeric(ctx: &LeafCtx<'_>, base: &BaseType, inner: LeafBuilder) -> Encoder {
...
fn option_for_numeric_widened(ctx: &LeafCtx<'_>, base: &BaseType, inner: LeafBuilder) -> Encoder {
...
fn option_for_bool(ctx: &LeafCtx<'_>, inner: LeafBuilder) -> Encoder {
    let buf = idents::primitive_buf(ctx.idx);
    let validity = idents::primitive_validity(ctx.idx);
    let row_idx = idents::primitive_row_idx(ctx.idx);
    let pa_root = crate::codegen::polars_paths::polars_arrow_root();
...
fn option_for_decimal(ctx: &LeafCtx<'_>, precision: u8, scale: u8, inner: LeafBuilder) -> Encoder {
    let buf = idents::primitive_buf(ctx.idx);
    let name = ctx.name;
    let pp = crate::codegen::polars_paths::prelude();
...
fn option_for_as_str(ctx: &LeafCtx<'_>, inner: LeafBuilder) -> Encoder {
    let buf = idents::primitive_buf(ctx.idx);
    let name = ctx.name;
    let pp = crate::codegen::polars_paths::prelude();
...
pub(super) fn wrap_option(shape: &LeafShape<'_>, inner: LeafBuilder, ctx: &LeafCtx<'_>) -> Encoder {
    match shape {
...
pub(super) fn wrap_multi_option_primitive(
    ...
    let builder = super::vec::build_leaf(shape, &new_ctx);
    let wrapped = wrap_option(shape, builder, &new_ctx);
    match wrapped {
        Encoder::Leaf {
            decls,
            push,
