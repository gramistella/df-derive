...
//!
//! - **Rust edition**: 2024
//! - **Polars**: 0.53 (tested). Enable Polars features `timezones` for `DateTime<Utc>`,
//!   `dtype-date` for `NaiveDate`, `dtype-time` for `NaiveTime`, `dtype-duration` for duration
//!   columns, and `dtype-decimal` for `Decimal`.
